...
// CommonJS module (module.exports) for Vercel compatibility
  ...
  try {
    if (!ai || ai === 'gemini') {
      ...
      if (!apiKey) {
        ...
        return res.status(500).json({ error: 'OPENAI_API_KEY not set in Vercel environment' });
      }
    ...
    console.error('[chat] exception:', err.message, err.stack);
